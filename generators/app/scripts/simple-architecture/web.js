exports.generateWeb = (context, props) => {
  ...
  if (props.isWebApi) {
    for (var i = 0; i < props.entitiesArray.length; i++) {
      context.fs.copyTpl(
        context.templatePath('WithoutAuthorization/WEB/ApiControllers/_ApiControllerTemplate.cs'),
        context.destinationPath(
          props.solutionName +
          "/" +
          props.solutionName +
    ...
    context.fs.copyTpl(
        context.templatePath('WithoutAuthorization/WEB/ApiControllers/ApiExampleController.cs'),
        context.destinationPath(
          props.solutionName +
          "/" +
          props.solutionName +
          ".WEB/" +
          "ApiControllers/ApiExampleController.cs"), {
          SolutionName: props.solutionName,
          Entity: props.entitiesArray[i]
        });
  }
